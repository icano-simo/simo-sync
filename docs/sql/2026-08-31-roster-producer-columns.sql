...
--
-- Las tres ya estan en la vista `hr_centralizado.roster_for_admin`.
...
-- ---------------------------------------------------------------------------
-- Pendiente de decidir: active_set_by_hand
-- ---------------------------------------------------------------------------
-- La vista tambien expone `active_set_by_hand`, el gemelo de
-- `producer_set_by_hand` para `is_active`, agregado en el mismo cambio. No se
-- mapea todavia porque no estaba en lo pedido. Sin el, la pantalla de Admin no
-- puede distinguir "activa segun el archivo de RRHH" de "activa porque alguien
-- lo decidio", que es la misma distincion que si se hace para producer.
--
-- alter table org.roster_current
